<script type="text/javascript">
    ...
    $(document).on('click', '.btnLihatPelatihan', function(e) {
        e.stopPropagation();
        var idopd = $(this).data('id');
        $('#modalDetailPelatihan').modal({
            backdrop: 'static'
        });
        getDataListPelatihan(idopd);
    });

    $(document).on('click', '.btnClose', function(e) {
        $('#modalDetailPelatihan').modal('toggle');
    });

    var dataRecord = [];
    function getDataListPelatihan(idopd) {
        let html = '';
        $.ajax({
            type: 'GET',
            url: site + '/review',
                ...
                if (data.status = 'RC200') {
                    if (Object.keys(data.message).length > 0) {
                        ...
                        $.each(data.message, function(row, v) {
                            dataRecord[no - 1] = [
                                no, v.nm_pelatihan, v.tempat_pelatihan, v.tanggal_pelatihan, v.kuota, v.tot_baruak
                            ];
                            no++;
                        });
                    ...
                    html = '<tr><td colspan="3"><i>Tidak Ada Data</i></td></tr>';
                $('#tblviewpelatihan > tbody').html(html);
            },
            complete: function() {
                $('#tblviewpelatihan').DataTable({
                    data: dataRecord,
                    destroy: true,
                    columnDefs: [
                        {
