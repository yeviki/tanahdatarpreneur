...
class Master_pelatihan extends SLP_Controller {
    protected $_vwName  = '';
    protected $_uriName = '';
    public function __construct() {
        parent::__construct();
        $this->load->model(array('model_master_pelatihan' => 'mMasterpelatihan', 'master/model_master' => 'mmas'));
        $this->_vwName = 'vmspelatihan';
        $this->_uriName = 'data/master-pelatihan';
    }
    ...
    public function index() {
        ...
        $this->breadcrumb->add('Master Pelatihan', site_url($this->_uriName));
        $this->session_info['page_name']        = 'Master Pelatihan';
        $this->session_info['siteUri']          = $this->_uriName;
        $this->session_info['page_js']	        = $this->load->view($this->_vwName.'/vjs', array('siteUri'=>$this->_uriName), true);
        $this->session_info['data_urusan']      = $this->mmas->getKatUrusanPemerintahan();
        ...
        if(!$this->input->is_ajax_request()) {
            ...
            if(isset($session)){
                ...
                foreach ($dataList as $key => $dl) {
                    ...
                    $row[] = $no;
                    $row[] = $dl['nm_pelatihan'];
                    $row[] = $dl['nm_kat_urusan'];
                    $row[] = $dl['id_master_pelatihan'];
                    $row[] = '<button type="button" class="btn btn-orange btn-sm px-2 py-1 my-0 mx-0 waves-effect waves-light btnEdit" data-id="'.$this->encryption->encrypt($dl['id_master_pelatihan']).'" title="Edit data"><i class="fas fa-pencil-alt"></i></button>
                    <button type="button" class="btn btn-danger btn-sm px-2 py-1 my-0 mx-0 waves-effect waves-light btnDelete" data-id="'.$this->encryption->encrypt($dl['id_master_pelatihan']).'" title="Hapus data"><i class="fas fa-trash-alt"></i></button>';
                    $data[] = $row;
                }
                $output = array(
    ...
    public function details() {
        if(!$this->input->is_ajax_request()) {
            ...
            if(!empty($contId) AND !empty($session)) {
                ...
                $row['id_kat_urusan']	= !empty($data) ? $data['id_kat_urusan'] : '';
