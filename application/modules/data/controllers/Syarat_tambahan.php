...
class Syarat_tambahan extends SLP_Controller {
    protected $_vwName  = '';
    protected $_uriName = '';
    public function __construct() {
        parent::__construct();
        $this->load->model(array('model_syarat_tambahan' => 'mSyaratTambah', 'master/model_master' => 'mmas'));
        $this->_vwName = 'vsyarat_tambahan';
        $this->_uriName = 'data/syarat-tambahan';
    }

    private function validasiDataValue() {
        $this->form_validation->set_rules('nm_syarat_dinamis', 'Nama Syarat', 'required|trim');
    ...
    public function index() {
        ...
        $this->breadcrumb->add('Syarat Tambahan', site_url($this->_uriName));
        $this->session_info['page_name']        = 'Syarat Tambahan';
    ...
    public function listview() {
        if(!$this->input->is_ajax_request()) {
            ...
            if(isset($session)){
                $dataList = $this->mSyaratTambah->get_datatables();
                $no = $this->input->post('start');
                foreach ($dataList as $key => $dl) {
                    $no++;
                    $row = array();
                    $row[] = $no;
                    $row[] = $dl['nm_syarat_dinamis'];
                    $row[] = '<button type="button" class="btn btn-orange btn-sm px-2 py-1 my-0 mx-0 waves-effect waves-light btnEdit" data-id="'.$this->encryption->encrypt($dl['id_syarat_dinamis']).'" title="Edit data"><i class="fas fa-pencil-alt"></i></button>
                    <button type="button" class="btn btn-danger btn-sm px-2 py-1 my-0 mx-0 waves-effect waves-light btnDelete" data-id="'.$this->encryption->encrypt($dl['id_syarat_dinamis']).'" title="Hapus data"><i class="fas fa-trash-alt"></i></button>';
                    $data[] = $row;
                }
                $output = array(
                    "draw" => $this->input->post('draw'),
                    "recordsTotal" => $this->mSyaratTambah->count_all(),
                    "recordsFiltered" => $this->mSyaratTambah->count_filtered(),
                    "data" => $data,
                );
            }
    ...
    public function details() {
        if(!$this->input->is_ajax_request()) {
            ...
            if(!empty($contId) AND !empty($session)) {
                $data = $this->mSyaratTambah->getDataDetail($this->encryption->decrypt($contId));
                $row = array();
                $row['nm_syarat_dinamis']	= !empty($data) ? $data['nm_syarat_dinamis'] : '';
        ...
        if(!$this->input->is_ajax_request()) {
            ...
            if(!empty($session) AND !empty($contId)) {
                if($this->validasiDataValue() == FALSE) {
                    ...
                    $data = $this->mSyaratTambah->updateData();
                    if($data['response'] == 'ERROR') {
    ...
    public function delete() {
        if(!$this->input->is_ajax_request()) {
            ...
            $contId   = escape($this->input->post('tokenId', TRUE));
            if(!empty($session) AND !empty($contId)) {
                $data = $this->mSyaratTambah->deleteData();
                if($data['response'] == 'ERROR') {
