...
if (!function_exists('fotoUser')) {
    function fotoUser($file) {
        if($file == null || $file == ""){
            return "default-user-icon.jpg";
        }else{
            if(file_exists(realpath('assets/img/avatar/'.$file))){
                return $file;
            }else{
                return "default-user-icon.jpg";
            }
...
/**
 * Fungsi get profile data peserta
 */
if (!function_exists('currentDataPeserta')) {
    function currentDataPeserta() {
        $ci =& get_instance();
        $ci->load->database();
        $ci->db->select('a.token,
                        a.nik,
                        a.nama_lengkap,
                        a.tempat_lhr,
                        a.tanggal_lhr,
                        ...
                        f.name AS regency,
                        ');
        $ci->db->where('a.token', $ci->app_loader->current_userid());
        $ci->db->join('ref_gender b', 'b.id_gender = a.id_gender', 'inner');
        $ci->db->join('ref_agama c', 'c.id_agama = a.id_agama', 'inner');
        $ci->db->join('ref_pendidikan d', 'd.id_study = a.id_study', 'inner');
        $ci->db->join('wa_province e', 'e.id = a.id_province', 'inner');
        $ci->db->join('wa_regency f', 'f.id = a.id_regency', 'inner');
        $row = $ci->db->get('data_peserta a');
        return $row->row_array();
    }
}
...
if (!function_exists('array_group_by')) {
    ...
    /**
    * Fungsi get kategori_pelatihan
    */
    if (!function_exists('kategori_pelatihan')) {
        function kategori_pelatihan($id) {
            $ci =& get_instance();
            $ci->load->database();
            $ci->db->where('id_jenis_kegiatan', $id);
            $row = $ci->db->get('ms_jenis_kegiatan')->row_array();
            return !empty($row) ? $row['nm_jenis_kegiatan'] : '';
        }
    }
    ...
    if (!function_exists('agama')) {
        function agama($id) {
            ...
            $row = $ci->db->get('ref_agama')->row_array();
            return !empty($row) ? $row['agama'] : '';
        }
    }
    ...
    if (!function_exists('pendidikan')) {
        function pendidikan($id) {
            $ci =& get_instance();
            $ci->load->database();
            $ci->db->where('id_study', $id);
            $row = $ci->db->get('ref_pendidikan')->row_array();
            return !empty($row) ? $row['study'] : '';
        }
    }
}
